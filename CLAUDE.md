...
- **`ai/pipeline.js`**: `reharmonize(client, piece, { style, intensity, maxPerSlot, maxTokens, effort, candidates })` → `{ analyzed, candidates, slots, chosen, scores, grid, originalGrid, gridParses, issues, problems, model, usage, promptVersion }`. Opțiunea `candidates` înlocuiește meniul generat (o folosește lock & regenerate din Faza 5 și testele). Singurul motiv de respingere a promisiunii e eroarea de apel; tot ce greșește modelul intră în `problems` și costă slotul, nu rularea: id inexistent, slot răspuns de două ori, intrare fără coordonate, slot inexistent, alegere pe un slot deja acoperit de un candidat pe două sloturi.
