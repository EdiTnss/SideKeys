...
- Excepțiile de la „zero dependențe" sunt două, amândouă foldere izolate cu `package.json` propriu, din care `src/` nu importă nimic: `worker/` (cu `wrangler` ca dev dependency, pentru rulare locală și deploy) și `mic-proto/` (cu `@spotify/basic-pitch`, care aduce TensorFlow.js — aprobat de Edi pe 2026-09-24, pentru prototipul de microfon). Aplicația din `src/` nu importă nimic din npm. O a treia excepție se cere explicit.
...
demo/manifest.json         mereu publicat: numește fișierele demo care există, sau null (fără 404 în consolă)
demo/piece.json            piesa demo (grilă + melodie); demo/reharm.json, răspunsul salvat — nepublicate încă
...
mic-proto/                 prototipul de microfon, măsurat și închis (verdict în docs/spec-mic.md);
                           folder izolat, cu package.json propriu; src/ nu importă nimic din el
...
assets/                    brandul: favicon, mărci, lockup-uri, imaginea OG, BRAND.md (specificația designerului);
                           fonts/ are Sora găzduit local, cu licența OFL.txt
```
...
- **239 teste** verzi, CI pe Node 22 și 24. Local, `npm run test:quiet`. Cele trei sesiuni înregistrate se reiau neschimbate.
...
- **Prototipul de microfon e măsurat și închis** (2026-09-24, spec în [docs/spec-mic.md](docs/spec-mic.md)): **45% pe cele 20 de voicings de poartă, față de pragul de 90%**, cu tavan de **72%** pe toate cele 93 — adică nici cea mai bună regulă de prag posibilă nu ajunge, fiindcă modelul nu raportează deloc notele lipsă. Corpusul nu e inventat: sunt voicings-urile reale ale lui Edi din sesiunea Genos, re-sintetizate cu timpii și velocitățile lor, plus un drum acustic simulat. Eșecul e „note lipsă", nu „note inventate" (precizie 96–99%), iar cazul care omoară ideea e octava dublă: într-un voicing cu două mâini, octava superioară reală vine mai slabă decât fantomele de octavă din alte acorduri, deci nu se pot deosebi. **Argumentul de produs bate procentul**: aplicația n-ar spune „n-am auzit bine", ar spune „îți lipsește cvinta" unui pianist care a cântat-o. Modul microfon din P4 rămâne închis până se schimbă modelul, nu regula de citire; `mic-proto/` rămâne în repo cu raportările.
...
- **Brandul e legat în pagină** (2026-09-22), făcut de Edi în claude.ai/design după [docs/design/BRIEF.md](docs/design/BRIEF.md); specificația designerului e în `assets/BRAND.md`. Accent magenta (`#E0489E` pe întunecat, `#C41E7F` pe luminos), neutre calde, marca cu patru bare dintre care ultima e coborâtă la margine, wordmark-ul Sora. **Culorile de teorie au rămas neatinse**, iar accentul stă doar pe elemente de interfață — lockup, tab activ, link-uri, focus — **niciodată pe o clapă sau într-un verdict**: la deuteranopie, accentul întunecat e la 6,5 ΔE de `--held`, deci regula e portantă (comentariul din `styles.css`). Măsurat: la vedere normală orice pereche accent–teorie e peste 23 ΔE, contrastul accentului e 5,24:1 pe ambele teme. **Imaginea OG spune adevărul** (repictată de Edi pe 2026-09-22): fiecare clapă colorată din bandă e exact ce ar spune aplicația pe Dm7b5, inclusiv conturul „caution" pe 9. Rămâne un Do gri („held") lângă culori de verdict, stare pe care aplicația n-o arată — neînsemnat, nu afirmă nimic fals. **Sora e găzduit local** în `assets/fonts/`: un singur fișier variabil, subsetul latin, axa întreagă (100–800, 33 KB), cu licența `OFL.txt` alături, cum cere OFL.
- **Neverificat pe Genos — riscul principal.** Tot ce s-a schimbat la pornirea MIDI (porturile virtuale ținute lângă cele reale prin `mergeAccess`, ieșirea implicită care preferă instrumentul, literele ca pian care se opresc când apare o clapă reală, cererea automată de Web MIDI doar pe `localhost`) și mutarea datelor din `localStorage` de sub `voicing-lab.*` sub `sidekeys.*` au fost verificate **doar pe portul virtual și în teste**. Sesiunea reală a lui Edi se reia neschimbat în harness, deci captura și verdictele sunt aceleași; nevăzute pe clapă sunt doar pornirea și datele lui.
- **De la Edi, pe lângă verificarea pe Genos**: GIF-ul de 15 secunde și videoul de 60–90 s pentru README (comentariul `<!-- demo.gif -->` marchează locul), domeniul `sidekeys.app`, confirmarea generalizării regulii de anticipare. **Deschis în produs, nu în cod**: poarta P0 cere 40 de adrese de email, iar în `index.html`, `README.md` și tot `src/` nu există niciun loc unde cineva să lase una — deci poarta nu se poate nici măsura, nici trece; cel mai ieftin drum e un formular găzduit, legat din README și din pagină, dar e o decizie de scop, nu o sarcină începută. **La P2, nu acum**: opinie de marcă pe familia „Sidekick" în clasele 9 și 41, înainte de înregistrare.
...
Cu Edi la Genos, pornește `start.cmd` și verifică pe clapa reală ce a fost testat până acum doar pe portul virtual: Genos-ul apare ca intrare și e ales singur ca ieșire MIDI (nu „Browser synth"), literele `A S D F…` nu cântă cât e conectat, iar piesele, setările și statisticile lui cumulative sunt încă acolo, acum sub cheile `sidekeys.*`.
...
| [docs/spec-mic.md](docs/spec-mic.md) | prototipul de microfon: ce s-a măsurat, cum, cifrele și verdictul | `mic-proto/`; dacă se reia vreodată modul microfon |
