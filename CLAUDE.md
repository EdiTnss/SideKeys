...
- **Rămas din P0**: **publicarea** — istoric verificat pentru secrete, licență, repo public, Pages, Worker publicat + `PUBLISHED_PROXY_URL` completat în `app.js`, README în engleză, GIF, video — apoi **prototipul de microfon** cu `basic-pitch-ts`, în afara lui `src/` (prag: peste 90% din 20 de voicings identificate corect, cu octava exactă). De la Edi, separat: o piesă proprie cu „plan & review" ascultată pe Genos (ultima jumătate a DoD-ului Fazei 3b).
...
Publicarea: verifică istoricul pentru secrete, propune-i lui Edi licența, apoi README-ul în engleză; părțile care cer contul lui (repo public, Pages, `wrangler login` și deploy) rămân ale lui, cu pașii scriși.
